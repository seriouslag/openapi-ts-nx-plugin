...
| `@hey-api/codegen-core`                               | ESM-only; keep in lockstep with openapi-ts | Same dynamic-`import()` constraint. Its `loadConfigFile` reads `plugins` out of `openapi-ts.config.*` so plugin options survive the merge into `createClient`. `@hey-api/openapi-ts` depends on an **exact** version of it, so when that bumps to a new minor this range must bump too — otherwise pnpm installs two copies and we read the config with a different loader than the generator uses, silently. `version:check:nx-plugin` enforces this: it compares our range against the version the installed openapi-ts resolves, and fails with the range to write. |
