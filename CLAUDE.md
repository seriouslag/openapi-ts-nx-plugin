...
| `c12`                                                 | ESM-only; must match hey-api's loader     | Same dynamic-`import()` constraint. Used to read `plugins` out of `openapi-ts.config.*` so plugin options survive the merge into `createClient` — it must load the file the same way `@hey-api/openapi-ts` does (extension stripped, `name: 'openapi-ts'`) or we merge against a different config than the generator uses. |
